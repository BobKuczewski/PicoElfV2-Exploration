# Disk Layout after installing ElfOS from the PE2 ROM

This section documents the approximate bytes on
the disk that were changed during various stages
of installation from the ROM to a 128MB CF Card.
...
The differences show the areas of the
disk written in each phase of the
...
Each of these XPM files were then converted to PNG format for this page.

Each image below uses black for areas that have not been changed during
...
also used to generate a cumulative image. These images are shown below.

## Results

After step 1, only the first 266 bytes or so of the disk have been changed:

![Step 1 Drive Initialization](CMP_0_1.png?raw=true "Step 1 Drive Initialization")

The changes in step 2 are a bit more extensive. There are quite a few changes to the first 512 bytes of the disk. Additionally there are many changes to the bytes from 8705 through 8742 and from 72449 through 73728:

![Step 2 Filesystem Generation](CMP_1_2.png?raw=true "Step 2 Filesystem Generation")

The changes in step 3 are limited to the first 8704 bytes of the disk. But nearly every byte in that range is changed:

![Step 3 System Installation (sys)](CMP_2_3.png?raw=true "Step 3 System Installation (sys)")

The changes in step 4 are mostly above 73732 bytes into the disk. There are a few changes (possibly directory entries?) to the lower sections between 306 and 309 and again between 8743 and 8760:
